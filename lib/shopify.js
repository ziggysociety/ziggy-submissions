...
async function setInventory(inventoryItemId, quantity) {
  ...
  // A brand-new variant's inventory item isn't stocked at any location yet, so
  // inventorySetQuantities alone fails silently. inventoryActivate connects the
  // item to the location AND sets the available quantity in one call.
  ...
  } catch (e) { /* may already be active at this location */ }
  // Set the on-hand quantity; "available" follows when nothing is committed.
  await gql(
    `mutation($input: InventorySetQuantitiesInput!) {
       inventorySetQuantities(input: $input) { userErrors { field message } }
     }`,
    { input: { reason: "correction", name: "on_hand", ignoreCompareQuantity: true,
...
async function createDraftProduct({ title, descriptionHtml, vendor, productType, tags, price, sku, imageUrls, photos, stock, variants, vendorEmail }) {
  ...
  if (multi) {
    ...
    for (const q of qtyByNode) { try { await setInventory(q.invId, q.qty); } catch (e) { /* non-fatal */ } }
  } else {
    const node = nodes[0];
