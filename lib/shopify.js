...
async function setInventory(inventoryItemId, quantity) {
  ...
  // Tracking is already enabled on the variant via productVariantsBulkUpdate/Create,
  // so we only need to stock it at a location and set the quantity here.
  const qty = parseQty(quantity);
  if (qty === null) return;
  const locationId = await getPrimaryLocationId();
  if (!locationId) throw new Error('no primary location found');
  // Ensure the item is stocked at the location (creates the inventory level).
  let actErr = [];
  const idemKey = `zg-act-${String(inventoryItemId).split("/").pop()}-${String(locationId).split("/").pop()}`;
  const act = await gql(
    `mutation($inventoryItemId: ID!, $locationId: ID!) {
       inventoryActivate(inventoryItemId: $inventoryItemId, locationId: $locationId) @idempotent(key: "${idemKey}") { inventoryLevel { id } userErrors { field message } }
     }`,
    { inventoryItemId, locationId }
  );
  actErr = (act.inventoryActivate && act.inventoryActivate.userErrors) || [];
  // Set the available quantity at that location.
  const setRes = await gql(
    `mutation($input: InventorySetQuantitiesInput!) {
       inventorySetQuantities(input: $input) @idempotent(key: "${idemKey}-set") { userErrors { field message } }
     }`,
    { input: { reason: "correction", name: "available", 
      quantities: [{ inventoryItemId, locationId, quantity: qty, changeFromQuantity: 0 }] } }
  );
  const setErr = (setRes.inventorySetQuantities && setRes.inventorySetQuantities.userErrors) || [];
  if (setErr.length) throw new Error('setQuantities=' + JSON.stringify(setErr) + ' activate=' + JSON.stringify(actErr) + ' loc=' + locationId);
}
...
async function createDraftProduct({ title, descriptionHtml, vendor, productType, tags, price, sku, imageUrls, photos, stock, variants, vendorEmail }) {
  ...
  const invErrors = [];
  if (multi) {
    // productCreate only guarantees a variant for the FIRST option value, so any
    // additional sizes must be created explicitly with productVariantsBulkCreate.
    let allNodes = nodes.slice();
    const optName = (productInput.productOptions && productInput.productOptions[0] && productInput.productOptions[0].name) || 'Size';
    const haveVals = new Set(allNodes.map(n => (n.selectedOptions && n.selectedOptions[0]) ? n.selectedOptions[0].value : null));
    const toCreate = variants.filter(v => !haveVals.has(String(v.name || '').trim() || 'Variant'));
    if (toCreate.length) {
      const createdVars = await gql(
        ...
        { productId: product.id, variants: toCreate.map(v => {
            const vin = { optionValues: [{ name: (String(v.name || '').trim() || 'Variant'), optionName: optName }] };
            const pp = parsePrice(v.price || price);
            if (pp) vin.price = pp;
            vin.inventoryItem = v.sku ? { sku: String(v.sku), tracked: true } : { tracked: true };
            return vin;
          }) }
      );
      const bcErr = createdVars.productVariantsBulkCreate.userErrors;
      if (bcErr && bcErr.length) throw new Error('Shopify variant create: ' + JSON.stringify(bcErr));
      allNodes = allNodes.concat(createdVars.productVariantsBulkCreate.productVariants || []);
    }
    ...
    for (const node of allNodes) {
      ...
      if (pp) vin.price = pp;
      updates.push(vin);
      const invId = node.inventoryItem && node.inventoryItem.id;
      if (invId && match.qty != null && String(match.qty) !== '') qtyByNode.push({ invId, qty: match.qty });
    }
    if (updates.length) await bulkUpdate(updates);
    // Set each size's own stock quantity at the primary location.
    for (const q of qtyByNode) { try { await setInventory(q.invId, q.qty); } catch (e) { invErrors.push(e.message); } }
  } else {
    const node = nodes[0];
    ...
    try { await setInventory(invItemId, stock); } catch (e) { invErrors.push(e.message); }
  }
  ...
  return { id: product.id, adminUrl, invErrors };
}
